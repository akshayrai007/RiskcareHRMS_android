...
import com.riskcare.app.databinding.ActivityMainBinding
import com.riskcare.app.ui.attendance.AttendanceFragment
import com.riskcare.app.ui.dashboard.DashboardFragment
import com.riskcare.app.ui.leave.LeaveFragment
import com.riskcare.app.ui.login.LoginActivity
import com.riskcare.app.ui.more.ApprovalsFragment
import com.riskcare.app.ui.more.MoreFragment
...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (sessionManager.isLoggedIn()) {
            com.riskcare.app.ui.attendance.OdTrackingWorker.scheduleDailyCheck(this)
            ...
            val trackingMgr = com.riskcare.app.domain.usecase.TrackingManager(this)
            val alreadyRunning = com.riskcare.app.service.LocationTrackingService.isRunning(this)
            val hasPerm        = com.riskcare.app.permission.PermissionManager.hasForegroundPermission(this)
            val isPunchedIn    = trackingMgr.isPunchedIn()

            if (isPunchedIn && !alreadyRunning && hasPerm) {
                val session = trackingMgr.getSession()
                com.riskcare.app.service.LocationTrackingService.start(this, isOd = session.isOd)
    ...
    private fun handleDeepLink(intent: Intent?) {
        ...
        when (path.lowercase()) {
            "approvals" -> {
                val frag = com.riskcare.app.ui.more.ApprovalsFragment()
                loadTab(frag)
    ...
    fun refreshDashboardBadge() {
        ...
        if (frag is com.riskcare.app.ui.dashboard.DashboardFragment) {
    ...
    override fun onBackPressed() {
        if (supportFragmentManager.backStackEntryCount > 0) {
            ...
            val current = supportFragmentManager.findFragmentById(R.id.navHostFragment)
            if (current !is com.riskcare.app.ui.dashboard.DashboardFragment) {
                binding.bottomNavView.selectedItemId = R.id.nav_dashboard
            } else {
