        ...
        view.findViewById<View>(R.id.rowLogout)?.setOnClickListener {
            android.app.AlertDialog.Builder(requireContext())
                ...
                .setPositiveButton("Sign Out") { _, _ ->
                    ...
                    com.riskcare.app.service.LocationTrackingService.stop(
                        requireContext(),
                        com.riskcare.app.domain.model.StopReason.SESSION_EXPIRED
                    )
                    com.riskcare.app.domain.usecase.TrackingManager(requireContext()).clearPunchCache()
                    SessionManager(requireContext()).clearSession()
                    val intent = android.content.Intent(requireContext(), com.riskcare.app.ui.login.LoginActivity::class.java)
            ...
            scope.launch {
                try {
                    val res = RetrofitClient.instance.addAnnouncementComment(
                        ann.id, com.riskcare.app.data.models.CommentRequest(text)
                    )
                    if (res.isSuccessful) {
    ...
    private fun showEditDialog(ctx: android.content.Context, advId: Int, currentAmount: Double, currentReason: String, onRefresh: () -> Unit) {
        ...
        androidx.appcompat.app.AlertDialog.Builder(ctx)
            ...
            .setPositiveButton("Save") { _, _ ->
                ...
                kotlinx.coroutines.GlobalScope.launch(kotlinx.coroutines.Dispatchers.Main) {
                    try {
                        val res = RetrofitClient.instance.editAdvance(advId, com.riskcare.app.data.models.AdvanceEditRequest(newAmount, newReason))
                        val ok = res.isSuccessful && res.body()?.success == true
        ...
        ll.setOnClickListener {
            ...
            when (notif.type?.lowercase()) {
                ...
                "birthday", "anniversary" -> {
                    activity.loadTab(com.riskcare.app.ui.dashboard.DashboardFragment())
                    activity.binding.bottomNavView.menu.findItem(R.id.nav_dashboard)?.isChecked = true
                }
                else -> {
                    activity.loadTab(ApprovalsFragment.newInstance(0))
